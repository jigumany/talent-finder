...
export function TourProvider({ children }: { children: ReactNode }) {
  ...
  const steps = (isMobile ? tourStepsMobile : TOUR_STEPS).map(step => {
      // @ts-ignore
      const { route, ...rest } = step;
      return rest;
  });
  ...
  const handleJoyrideCallback = (data: CallBackProps) => {
    ...
    const allSteps = isMobile ? tourStepsMobile : TOUR_STEPS;
    ...
    if (finishedStatuses.includes(status)) {
      ...
      setStepIndex(0);
        ...
        // @ts-ignore
        const nextStepRoute = allSteps[newIndex]?.route;
        
        if (nextStepRoute && pathname !== nextStepRoute) {
            router.push(nextStepRoute);
             // We need to pause the tour and then resume it after navigation
            setRun(false); 
            setTimeout(() => {
